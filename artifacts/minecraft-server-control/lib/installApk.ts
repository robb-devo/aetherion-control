...
import { isTrustedApkUrl, releaseSizeMatches } from '@/lib/appUpdate';
...
export async function downloadReleaseApk(
  ...
  if (Platform.OS !== 'android') {
    throw new Error('APK installs run on Android. Open the GitHub release on this device instead.');
  }
  if (!isTrustedApkUrl(url)) {
    throw new Error('Refusing to download an APK that is not attached to this repository\'s GitHub releases.');
  }
  ...
  if (!releaseSizeMatches(file.size, expectedSize)) {
    throw new Error(
      `Downloaded ${file.size} bytes, but the GitHub asset is ${expectedSize} bytes. The installer was not opened.`,
    );
  }
