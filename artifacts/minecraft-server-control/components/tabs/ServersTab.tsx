...
export default function ServersScreen() {
  ...
  return (
      ...
      <ScrollView refreshControl={<RefreshControl refreshing={isLoading || hostsQuery.isRefetching} onRefresh={() => void Promise.all([refresh(), hostsQuery.refetch()])} tintColor={colors.primary} colors={[colors.primary]} />} contentContainerStyle={[uiStyles.scroll, { paddingTop: insets.top + 16 }]}>
        ...
        {!error && (summary.partial || summary.stale) ? <View style={[styles.errorCard, { backgroundColor: colors.secondary, borderColor: colors.border }]}><Feather name="info" size={16} color={colors.primary} /><Text style={[styles.errorText, { color: colors.foreground }]}>{summary.stale ? 'Some stats are cached from the last Crafty response. They are not live zeros.' : 'Some servers did not return stats. They are not shown as offline zeros.'}</Text></View> : null}
        ...
        <View style={styles.list}>
          {(hostsQuery.data?.hosts ?? []).map((host) => (
            <View key={host.id} style={[styles.hetznerCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
              ...
              <View style={styles.hetznerBottom}><View><Text style={[styles.hetznerCost, { color: colors.foreground }]}>{host.serverIp}</Text><Text style={[styles.hetznerMeta, { color: colors.mutedForeground }]}>Traffic {host.traffic} · {host.paidUntil}</Text></View><Text style={[styles.hetznerLink, { color: host.status === 'ready' ? colors.success : colors.warning }]}>{host.status.toUpperCase()}</Text></View>
            </View>
          ))}
          {hostsQuery.isError ? <View style={[styles.errorCard, { backgroundColor: colors.secondary, borderColor: colors.destructive }]}><Feather name="alert-circle" size={16} color={colors.destructive} /><Text style={[styles.errorText, { color: colors.foreground }]}>{hostsQuery.error instanceof Error ? hostsQuery.error.message : 'Hetzner Robot could not be reached.'}</Text></View> : null}
          {!hostsQuery.isError && !(hostsQuery.data?.hosts?.length) && !hostsQuery.isLoading ? (
            <View style={[styles.errorCard, { backgroundColor: colors.secondary, borderColor: colors.border }]}>
              <Feather name="info" size={16} color={colors.primary} />
              <Text style={[styles.errorText, { color: colors.foreground }]}>No dedicated host data yet.</Text>
            </View>
          ) : null}
        </View>
      </ScrollView>
...
function ServerDetail({ server, onRestart, onOpen }: { server: MinecraftServer; onRestart: () => void; onOpen: () => void }) {
  const colors = useColors();
  const statusLabel = server.status === 'online' ? 'Operational' : server.status === 'restarting' ? 'Restarting…' : server.status === 'unknown' ? 'Stats unavailable' : 'Offline';
  const statusColor = server.status === 'offline' ? colors.destructive : server.status === 'restarting' ? colors.info : server.status === 'unknown' ? colors.mutedForeground : colors.success;
  const slots = server.players != null && server.maxPlayers ? Math.round((server.players / server.maxPlayers) * 100) : null;
  return <Pressable onPress={onOpen} style={({ pressed }) => [styles.serverCard, { backgroundColor: colors.card, borderColor: colors.border, opacity: pressed ? 0.82 : 1 }]}><View style={styles.serverTop}><View style={[styles.nodeIcon, { backgroundColor: server.status === 'offline' ? colors.secondary : colors.accent }]}><Feather name={server.tag === 'EDGE' ? 'shuffle' : 'box'} size={20} color={server.status === 'offline' ? colors.destructive : colors.primary} /></View><View style={{ flex: 1 }}><View style={styles.nameRow}><Text style={[styles.serverName, { color: colors.foreground }]}>{server.name}</Text><Text style={[styles.serverTag, { color: colors.mutedForeground }]}>{server.tag}</Text></View><View style={styles.statusRow}><StatusDot status={server.status} /><Text style={[styles.statusText, { color: statusColor }]}>{statusLabel}</Text></View></View><Pressable onPress={(event) => { event.stopPropagation(); onRestart(); }} hitSlop={10} style={({ pressed }) => [styles.restartButton, { backgroundColor: colors.secondary }, pressed && { opacity: 0.7 }]}><Feather name="rotate-cw" size={15} color={colors.foreground} /></Pressable></View><View style={styles.serverMetrics}><Metric label="CPU" value={server.cpu} color={(server.cpu ?? 0) > 65 ? colors.warning : colors.primary} /><Metric label="RAM" value={server.ram} color={(server.ram ?? 0) > 70 ? colors.warning : colors.info} /><Metric label="Slots" value={slots} color={colors.success} /></View><View style={styles.detailFooter}><Text style={[styles.footerText, { color: colors.mutedForeground }]}>{server.metricsKnown ? `${server.players ?? '—'}/${server.maxPlayers ?? '—'} players · ${server.ip}` : `Stats unavailable · ${server.ip}`}</Text><Text style={[styles.footerText, { color: colors.mutedForeground }]}>{server.metricsKnown ? `${server.statsFresh ? (server.uptime ?? '—') : 'Cached'}${server.memoryLabel ? ` · ${server.memoryLabel}` : ''}` : '—'}</Text></View>{server.status === 'restarting' ? <View style={{ marginTop: 13 }}><PrimaryButton label="Waiting for Crafty…" disabled /></View> : null}</Pressable>;
}
